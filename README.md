...
`LAMMPS_inpute` contains 197 simulation cases, each with a `TbHz.in` LAMMPS input script and a matching `TbHz.data` structure/topology file. These comprise 196 high-throughput parameter combinations and one additional MD validation case for the optimized nanochannel.

The repository contains **9 Python scripts** for data analysis and figure reproduction. The `data.csv` file is required by the Python workflow and should be kept in the same directory as the scripts.
...
The scripts use Python 3 with `numpy`, `pandas`, and `Pillow`.
...
The scripts write derived files to `outputs/gpr_selectivity/`.
